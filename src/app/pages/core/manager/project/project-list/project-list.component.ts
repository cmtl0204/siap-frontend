...
export class ProjectListComponent implements OnInit {
    ...
    findProjects(page: number = 1) {
        this._projectHttpService.findAll(page).subscribe({
            next: (data) => {
                this.items = data;
            }
        });
    ...
    buildButtonActions(item: ProjectInterface) {
        this.buttonActions = [
            ...
            },
            {
                ...documentButtonAction,
                command: () => {
                    if (this.selectedItem?.id) this.redirectProjectDocument(this.selectedItem.id);
                }
            }
        ];
    }
    ...
    redirectViewProject(id: string) {
        this._router.navigateByUrl(`${MY_ROUTES.corePages.manager.project.form.absolute}/${id}`);
    }

    redirectProjectDocument(id: string) {
        this._router.navigateByUrl(`${MY_ROUTES.corePages.manager.project.document.absolute}/${id}`);
    }

    redirectCreateProject() {
        this._router.navigateByUrl(`${MY_ROUTES.corePages.manager.project.form.absolute}/new`);
    }
}
