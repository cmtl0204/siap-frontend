...
export class StrategicPlanListComponent implements OnInit {
    ...
    redirectViewStrategicPlan(id: string) {
        this._router.navigateByUrl(`${MY_ROUTES.corePages.operator.strategicPlan.absolute}/${id}`);
    }

    redirectProjectDocument(id: string) {
        this._router.navigateByUrl(`${MY_ROUTES.corePages.operator.strategicPlan.document.absolute}/${id}`);
    }

    redirectCreateProject() {
        this._router.navigateByUrl(`${MY_ROUTES.corePages.operator.strategicPlan.form.absolute}/new`);
    }
}
